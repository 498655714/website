        ...
        <div class="layui-card">
            <div class="layui-form layui-card-header layuiadmin-card-header-auto">
                <div class="layui-form-item">
                    ...
                    <div class="layui-inline">
                        <button id="permissions_list_search" class="layui-btn layuiadmin-btn-admin" lay-submit lay-filter="permissions_list_search">
            ...
            <div class="layui-card-body">
                <div style="padding-bottom: 10px;">
                    <button class="layui-btn layuiadmin-btn-admin" data-type="add">添加新权限</button>
                </div>
    ...
    <script>
        layui.config({
            ...
            table.render({
                ...
                ,height: 500 //容器高度
                ,method:'post'
                ,url: "{{ route('admin.permissions.getData') }}" //数据接口
                ,page: true //开启分页
            ...
            //监听搜索
            form.on('submit(permissions_list_search)', function(data){
            ...
            //添加权限js
            @include('admin.permissions._createjs');

            //监听工具条
            table.on('tool(permission)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                var data = obj.data //获得当前行数据
                    ,layEvent = obj.event; //获得 lay-event 对应的值
                var permissions_id = data.id;
                if(layEvent === 'edit'){//编辑操作
                    layer.open({
                        type: 2
                        ,title: '编辑权限'
                        ,content: "permissions/"+permissions_id+'/edit'
                        ,maxmin: true
                        ,area: ['560px', '550px']
                        ,btn: ['确定', '取消']
                        ,yes: function(index, layero){
                            var iframeWindow = window['layui-layer-iframe'+ index]
                                ,submitID = 'permissions_edit'
                            ...
                            iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
                                ...
                                $.ajax({
                                    url:"permissions/"+permissions_id
                                    ,type:'post'
                                    ,data: field
